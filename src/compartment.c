    ...
    void *new_comp_base)
{
    ...
    for (size_t i = 0; i < comp_ehdr.e_phnum; ++i)
    {
        ...
        new_comp->segs = realloc(new_comp->segs,
    ...
    for (size_t i = 0; i < new_comp->lib_deps_count; ++i)
    {
        ...
        new_comp->mem_top = (char *) curr_dep->lib_mem_base
            + (uintptr_t) curr_dep->lib_segs[curr_dep->lib_segs_count - 1]
                  ->mem_top;
    }
    ...
    do_pread((int) new_comp->fd, comp_strtb, comp_strtb_hdr.sh_size,
        comp_strtb_hdr.sh_offset);

    Elf64_Sym *comp_symtb = malloc(comp_symtb_shdr.sh_size);
    do_pread((int) new_comp->fd, comp_symtb, comp_symtb_shdr.sh_size,
        comp_symtb_shdr.sh_offset);

    // Find symbols for entry_points
    Elf64_Sym *ep_syms
        = find_symbols((const char **) entry_points, entry_point_count, true,
    ...
    for (size_t i = 0; i < intercept_count; ++i)
    {
        ...
        {
            comp_add_intercept(new_comp, intercept_syms[i].st_value,
...
comp_map(struct Compartment *to_map)
{
    ...
    for (size_t i = 0; i < to_map->seg_count; ++i)
    {
        ...
        map_result = mmap((void *) curr_seg->mem_bot, curr_seg->mem_sz,
            /*curr_seg->prot_flags,*/ // TODO currently need read/write to
                                      // inject the intercepts, consider better
                                      // option
    ...
    for (size_t i = 0; i < to_map->rela_maps_count; ++i)
    {
        ...
        memcpy((void *) to_map->rela_maps[i].rela_address,
            &to_map->rela_maps[i].target_func_address, sizeof(void *));
    }
...
init_comp_scratch_mem(struct Compartment *new_comp)
{
    new_comp->scratch_mem_base
        = align_up((char *) new_comp->segs[new_comp->seg_count - 1]->mem_top
                + new_comp->page_size,
            new_comp->page_size);
...
init_lib_dep_info(struct LibDependency *lib_dep, struct Compartment *new_comp)
{
    ...
    lib_dep->lib_mem_base = align_down(
    ...
    for (size_t i = 0; i < lib_ehdr.e_shnum; ++i)
    {
        ...
        struct LibDependencySymbol *ld_syms = malloc(
            lib_dep->lib_syms_count * sizeof(struct LibDependencySymbol));
        ...
        ld_syms = realloc(
            ld_syms, actual_syms * sizeof(struct LibDependencySymbol));
